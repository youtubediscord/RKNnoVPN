...
1. Magisk/KernelSU/APatch-модуль с root-демоном `daemon`, CLI `daemonctl`, скриптами iptables и бинарником `sing-box`.
2. Android APK, который управляет демоном через `su -c daemonctl`; HTTP/control-plane операции выполняет root-демон, чтобы APK не зависел от собственной сети и Android VPN stack.
...
- подключается к Unix socket `/data/adb/modules/rknnovpn/run/daemon.sock`;
- печатает JSON-ответ;
...
- хранить и валидировать `/data/adb/modules/rknnovpn/config/config.json`;
...
4. `service.sh` ждёт boot completed и стартует `daemon`.
5. `daemon` поднимает Unix socket.
6. APK подключается через `daemonctl`.
7. При команде `start` демон рендерит `singbox.json`, запускает `sing-box`, ждёт порт `10853`, применяет iptables и DNS.

Если `sing-box` завершается до открытия порта, `daemon` возвращает ошибку с хвостом `/data/adb/modules/rknnovpn/logs/sing-box.log`.

Root runtime policy lives in `daemon/internal/runtime/root`: backend lifecycle,
desired-state projection, health classification, egress probe policy and node
probe orchestration. `daemon/cmd/daemon` wires real managers, locks and network
IO into that package, but should not own root-runtime policy decisions itself.
...
```text
/data/adb/modules/rknnovpn/config/profile.json
```
...
`version.supported_methods`, `diagnostics.report`, `daemonctl help` и APK compatibility checks
...
Все daemon mutations, которые меняют user intent (`profile.apply`,
`profile.importNodes`, `profile.setActiveNode`, `subscription.refresh`,
`config-import` и desired-state persist), проходят через единый
`persistProfileConfigMutation` path: validate/render готовят draft, затем daemon
сохраняет `profile.json`, применяет `config.json` projection, публикует
operation generation/status через runtime owner и возвращает typed transaction
result. `internal/apply` владеет mapping `action -> runtime operation kind`,
поэтому отдельные handlers не должны сами решать, был ли saved/apply/rollback
успешным или какой runtime operation надо запускать.
...
APK не требует `config-import` как compatibility prerequisite: full config import
остаётся daemon/CLI surface, а приложение работает через profile/subscription
intent methods и canonical `SubscriptionSource`, возвращённый daemon.
APK не держит локальную таблицу method -> capability: `ipc.contract` и
`backend.status.compatibility.methods` являются единственными источниками этого
mapping для compatibility checks.
...
`<dataDir>/run/install_state.json`: generation, artifacts, текущий step,
...
```sh
daemonctl diagnostics.testNodes
```
