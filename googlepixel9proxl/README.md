...
диагностика `doctor`, package counts и другие приватные данные телефона.
...
`privctl` уже установлен, вызывает `status`, `self-check` и `doctor`.
...
reboot. Он проверяет только доступность `privctl`, `status`, `self-check`,
`doctor` и структуру doctor JSON.
...
- После любого `--allow-start` делать `--stop-after-start` или отдельный
  `privctl stop`, если цель теста не требует оставить routing включенным.
- При странном состоянии сначала собирать новый `doctor`, а не чинить руками.
